<?php namespace Tests\APIs;

use Illuminate\Foundation\Testing\WithoutMiddleware;
use Illuminate\Foundation\Testing\DatabaseTransactions;
use Tests\TestCase;
use Tests\ApiTestTrait;
use App\Models\UserRegistration;

class UserRegistrationApiTest extends TestCase
{
    use ApiTestTrait, WithoutMiddleware, DatabaseTransactions;

    /**
     * @test
     */
    public function test_create_user_registration()
    {
        $userRegistration = factory(UserRegistration::class)->make()->toArray();

        $this->response = $this->json(
            'POST',
            '/api/user_registrations', $userRegistration
        );

        $this->assertApiResponse($userRegistration);
    }

    /**
     * @test
     */
    public function test_read_user_registration()
    {
        $userRegistration = factory(UserRegistration::class)->create();

        $this->response = $this->json(
            'GET',
            '/api/user_registrations/'.$userRegistration->id
        );

        $this->assertApiResponse($userRegistration->toArray());
    }

    /**
     * @test
     */
    public function test_update_user_registration()
    {
        $userRegistration = factory(UserRegistration::class)->create();
        $editedUserRegistration = factory(UserRegistration::class)->make()->toArray();

        $this->response = $this->json(
            'PUT',
            '/api/user_registrations/'.$userRegistration->id,
            $editedUserRegistration
        );

        $this->assertApiResponse($editedUserRegistration);
    }

    /**
     * @test
     */
    public function test_delete_user_registration()
    {
        $userRegistration = factory(UserRegistration::class)->create();

        $this->response = $this->json(
            'DELETE',
             '/api/user_registrations/'.$userRegistration->id
         );

        $this->assertApiSuccess();
        $this->response = $this->json(
            'GET',
            '/api/user_registrations/'.$userRegistration->id
        );

        $this->response->assertStatus(404);
    }
}
