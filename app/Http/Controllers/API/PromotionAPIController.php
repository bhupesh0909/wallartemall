    ...
    public function PromoCode(Request $request)
    {
        try {
            ...
            $validator = Validator::make($request->all(), [
                 ...
                 'amount'=>'required|integer|between:1,50000',
                 'promo_code'=>'required|max:15', 
                 
             ],[
            
                'promo_code.max' => 'Promocode should be less then 15 characters',
            ]);
            ...
            if (!$validator->passes()) {
             ...
             $check_promo_exits = $this->promotionRepository->CheckPromo($request->promo_code);

             if(!empty($check_promo_exits)){

                $promo_expiry_date = strtotime($check_promo_exits->promo_expiry);
                $current_date = strtotime(date('Y-m-d'));             

                if($current_date > $promo_expiry_date)
                {
                    return response::json(['status' => 0, 'message' => ["Promo code is expired"]]);
                }else{

                    if($request->amount < $check_promo_exits->criteria){
                        return response::json(['status' => 0, 'message' => ["You need to deposit minimum Rs.".$check_promo_exits->criteria]]);
                    }else{
                         return response::json(['status' => 1, 'message' => ["Congrats! you will get for Rs ".$check_promo_exits->chips." extra on deposit of ".$request->amount]]);
                    } 
                    
                }
             }else{
                return response::json(['status' => 0, 'message' => ["Invalid Promo Code"]]);
             }
