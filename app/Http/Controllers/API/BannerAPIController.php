...
class BannerAPIController extends AppBaseController
{
    ...
    public function index(Request $request)
    {
        ...
        $banners = $this->bannerRepository->GetBannerList();
        $i = 0;
        foreach ($banners as $o) {
            $banners[$i++]['banner_image'] = url('public/banner_image/') . '/' . $o['banner_image'];
        }
        
        return response::json(['status' => 1, 'banner' => $banners, 'message' => ['Get Banners successfully.']]);
    }
